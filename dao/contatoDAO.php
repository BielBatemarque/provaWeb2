...
class ContatoDAO
{
	...
	public function buscarContatoPorCodigo($codigo){
		$comandoSQL = 'select * from contato where id = :codigo';
	...
	public function inserirResposta($msg){
		try{
			...
			VALUES (:mensagem, :id_contato)';
			$stmt = $this->conn->prepare($comandoSQL);
			$resultado=$stmt->execute(array(
			    ':mensagem' => $msg->getDescricao(),
			    ':id_contato' => $msg->getCodigo()));
			$this->conn->commit();
		}catch(PDOException $e){
			$this->conn->rollback();
	...
	public function excluirContato($id){
		$comandoSQL = "delete from resposta where id_contato = :id";    
		$stmt = $this->conn->prepare($comandoSQL);
